...
2. `client.gui.screen.base` 接管 Minecraft `Screen` 运行时；`BaseTabbedScreen` 只做事件/生命周期桥接，具体运行状态由包内协作者持有。
...
- `PageViewport` 保存未滚动的基础坐标，每次刷新都从基础坐标重新计算，不能累计修改坐标。
...
`BaseTabbedScreen` 只做 Minecraft 事件和生命周期桥接，不再充当所有屏幕能力的实现容器。
包内协作者各自拥有明确状态：

- `TabStripController`：页签测量、布局、溢出窗口、翻页和选中状态。
- `PageStackController`：页面定义、活动页生命周期和事件路由。
- `PageViewport`：控件坐标、裁剪、滚动和展开式 overlay。
- `CloseCoordinator`：未保存提示和关闭确认。
- `ScreenModal`、`ToastController`、`BottomBarBackdropWidget`：临时呈现状态。

这些协作者保持 package-private，公共屏幕 API 不变。不要为了少写一个桥接方法而把状态重新塞回
`BaseTabbedScreen`。

`UIForm` 是唯一面向调用方的表单模型，同时直接持有共享布局、状态、交互路由和生命周期数据。它把控件创建委托给
`FormInputFactory` 和 `FormDisplayFactory`。新增控件时应在 factory 中实现，并只通过 `UIForm` 暴露一次；内部不再维护平行的表单模型。
