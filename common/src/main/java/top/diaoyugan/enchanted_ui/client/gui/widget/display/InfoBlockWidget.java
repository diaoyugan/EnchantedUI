...
public class InfoBlockWidget extends AbstractWidget {
    ...
    public InfoBlockWidget(int x, int y, int width, int height, Component title, Component message, int accentColor) {
        super(x, y, width, Math.max(
                height,
                DisplayText.wrappedBlockHeight(Minecraft.getInstance(), title, message, width - 16, 7, 4, 7)
        ), title);
    ...
    protected void extractWidgetRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        DisplayText.renderWrapped(
                guiGraphics, minecraft, message, x + 8, bodyY, width - 16, 0xFFBDBDBD, false
        );
    }
